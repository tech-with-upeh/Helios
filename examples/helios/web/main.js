...
var ASM_CONSTS = {
  87428: ($0) => { document.body.style = allocateUTF8(""); document.body.innerHTML = UTF8ToString($0); },  
 87516: () => { document.querySelectorAll("[data-ink-id]").forEach(el => { Module.domCache[el.dataset.inkId] = el; }); },  
 87623: () => { document.querySelectorAll("[data-ink-id]").forEach(el => { Module.domCache[el.dataset.inkId] = el; const cbId = el.dataset.callback; if(cbId) { el.onclick = () => Module._invokeVNodeCallback(cbId); } }); },  
 87831: ($0) => { document.title = UTF8ToString($0); },  
 87870: ($0, $1) => { if (!document.getElementById("__ink_styles")) { const style = document.createElement("style"); style.id = "__ink_styles_" + UTF8ToString($0); style.innerHTML = UTF8ToString($1); document.head.appendChild(style); } },  
 88088: ($0) => { const style = document.getElementById("__ink_styles_" + UTF8ToString($0)); if (style) { style.remove(); } },  
 88198: ($0, $1) => { if ($1 == 1){ const script = document.createElement("script"); script.src = UTF8ToString($0); document.head.appendChild(script); } else { const linkElem = document.createElement("link"); linkElem.rel = "stylesheet"; linkElem.href = UTF8ToString($0); document.head.appendChild(linkElem); } },  
 88491: ($0, $1, $2) => { const url = UTF8ToString($0); if($2 == 1) { const scripts = document.querySelectorAll("script[src]"); scripts.forEach(s => { s.remove(); }); const links = document.querySelectorAll("link[href]"); links.forEach(l => { l.remove(); }); } else { if ($1 == 1) { const scripts = document.querySelectorAll("script[src]"); scripts.forEach(s => { if (s.src === url || s.getAttribute("src") === url) { s.remove(); } }); } else { const links = document.querySelectorAll("link[href]"); links.forEach(l => { if (l.href === url || l.getAttribute("href") === url) { l.remove(); } }); } } },  
 89068: ($0) => { const linkElem = document.createElement("link"); linkElem.rel = "icon"; linkElem.href = UTF8ToString($0); linkElem.type = "image/x-icon"; document.head.appendChild(linkElem); },  
 89247: ($0, $1) => { document.body.setAttribute(UTF8ToString($0), UTF8ToString($1)); },  
 89315: ($0, $1) => { const id = UTF8ToString($0); if (!document.getElementById(id)) { document.body.insertAdjacentHTML("beforeend", UTF8ToString($1)); } },  
 89451: () => { function rafLoop() { Module._animatefps(); requestAnimationFrame(rafLoop); } requestAnimationFrame(rafLoop); },  
 89564: ($0) => { window.addEventListener(UTF8ToString($0), function () { Module._handleEvent($0); }); },  
 89653: ($0, $1) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (el) el.textContent = UTF8ToString($1); },  
 89779: ($0, $1, $2) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (!el) return; el.setAttribute(UTF8ToString($1), UTF8ToString($2)); },  
 89932: ($0, $1) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (!el) return; el.removeAttribute(UTF8ToString($1)); },  
 90070: ($0, $1) => { el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (el) { el.attributes['data-ink-id'] = UTF8ToString($1); } },  
 90208: () => { Module.domCache = {}; },  
 90234: () => { Module._handleRoute(allocateUTF8(window.location.pathname)); window.addEventListener("popstate", () => { Module._handleRoute(allocateUTF8(window.location.pathname)); }); },  
 90408: ($0, $1) => { const el = Module.domCache[UTF8ToString($0)]; if(el) el.textContent = UTF8ToString($1); },  
 90500: ($0, $1, $2) => { const el = Module.domCache[UTF8ToString($0)]; if(el) el.setAttribute(UTF8ToString($1),UTF8ToString($2)); },  
 90609: ($0, $1) => { const el = Module.domCache[UTF8ToString($0)]; if(el) el.removeAttribute(UTF8ToString($1)); },  
 90704: ($0, $1) => { const parent = Module.domCache[UTF8ToString($0)]; if(parent){ parent.insertAdjacentHTML("beforeend",UTF8ToString($1)); parent.querySelectorAll("[data-ink-id]").forEach(el=>{ Module.domCache[el.dataset.inkId]=el; const cbId = el.dataset.callback; if(cbId) { el.onclick = () => Module._invokeVNodeCallback(cbId); } }); } },  
 91027: ($0) => { const el = Module.domCache[UTF8ToString($0)]; if(el) el.remove(); },  
 91097: () => { console.log(document.body.getBoundingClientRect().height); return document.body.getBoundingClientRect().height; }
};
