...
var ASM_CONSTS = {
  85412: ($0) => { document.body.style = allocateUTF8(""); document.body.innerHTML = UTF8ToString($0); },  
 85500: ($0) => { document.title = UTF8ToString($0); },  
 85539: ($0, $1) => { if (!document.getElementById("__ink_styles")) { const style = document.createElement("style"); style.id = "__ink_styles_" + UTF8ToString($0); style.innerHTML = UTF8ToString($1); document.head.appendChild(style); } },  
 85757: ($0) => { const style = document.getElementById("__ink_styles_" + UTF8ToString($0)); if (style) { style.remove(); } },  
 85867: ($0, $1) => { if ($1 == 1){ const script = document.createElement("script"); script.src = UTF8ToString($0); document.head.appendChild(script); } else { const linkElem = document.createElement("link"); linkElem.rel = "stylesheet"; linkElem.href = UTF8ToString($0); document.head.appendChild(linkElem); } },  
 86160: ($0, $1, $2) => { const url = UTF8ToString($0); if($2 == 1) { const scripts = document.querySelectorAll("script[src]"); scripts.forEach(s => { s.remove(); }); const links = document.querySelectorAll("link[href]"); links.forEach(l => { l.remove(); }); } else { if ($1 == 1) { const scripts = document.querySelectorAll("script[src]"); scripts.forEach(s => { if (s.src === url || s.getAttribute("src") === url) { s.remove(); } }); } else { const links = document.querySelectorAll("link[href]"); links.forEach(l => { if (l.href === url || l.getAttribute("href") === url) { l.remove(); } }); } } },  
 86737: ($0) => { const linkElem = document.createElement("link"); linkElem.rel = "icon"; linkElem.href = UTF8ToString($0); linkElem.type = "image/x-icon"; document.head.appendChild(linkElem); },  
 86916: ($0, $1) => { document.body.setAttribute(UTF8ToString($0), UTF8ToString($1)); },  
 86984: ($0, $1) => { const id = UTF8ToString($0); if (!document.getElementById(id)) { document.body.insertAdjacentHTML("beforeend", UTF8ToString($1)); } },  
 87120: () => { function rafLoop() { Module._animatefps(); requestAnimationFrame(rafLoop); } requestAnimationFrame(rafLoop); },  
 87233: ($0) => { window.addEventListener(UTF8ToString($0), function () { Module._handleEvent($0); }); },  
 87322: ($0, $1) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (el) el.textContent = UTF8ToString($1); },  
 87448: ($0, $1, $2) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (!el) return; el.setAttribute(UTF8ToString($1), UTF8ToString($2)); },  
 87601: ($0, $1) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (!el) return; el.removeAttribute(UTF8ToString($1)); },  
 87739: ($0, $1) => { el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (el) { el.attributes['data-ink-id'] = UTF8ToString($1); } },  
 87877: () => { Module._handleRoute(allocateUTF8(window.location.pathname)); window.addEventListener("popstate", () => { Module._handleRoute(allocateUTF8(window.location.pathname)); }); },  
 88051: ($0) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (el) el.remove(); },  
 88155: ($0, $1) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (el) el.outerHTML = UTF8ToString($1); },  
 88279: ($0, $1) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (el) el.outerHTML = UTF8ToString($1); },  
 88403: ($0, $1) => { const parent = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (parent) parent.insertAdjacentHTML('beforeend', UTF8ToString($1)); },  
 88560: ($0) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (el) el.remove(); }
};
