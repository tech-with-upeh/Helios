...
var ASM_CONSTS = {
  86300: ($0) => { document.body.style = allocateUTF8(""); document.body.innerHTML = UTF8ToString($0); },  
 86388: ($0) => { document.title = UTF8ToString($0); },  
 86427: ($0, $1) => { if (!document.getElementById("__ink_styles")) { const style = document.createElement("style"); style.id = "__ink_styles_" + UTF8ToString($0); style.innerHTML = UTF8ToString($1); document.head.appendChild(style); } },  
 86645: ($0) => { const style = document.getElementById("__ink_styles_" + UTF8ToString($0)); if (style) { style.remove(); } },  
 86755: ($0, $1) => { if ($1 == 1){ const script = document.createElement("script"); script.src = UTF8ToString($0); document.head.appendChild(script); } else { const linkElem = document.createElement("link"); linkElem.rel = "stylesheet"; linkElem.href = UTF8ToString($0); document.head.appendChild(linkElem); } },  
 87048: ($0) => { const linkElem = document.createElement("link"); linkElem.rel = "icon"; linkElem.href = UTF8ToString($0); linkElem.type = "image/x-icon"; document.head.appendChild(linkElem); },  
 87227: ($0, $1) => { document.body.setAttribute(UTF8ToString($0), UTF8ToString($1)); },  
 87295: ($0, $1) => { const id = UTF8ToString($0); if (!document.getElementById(id)) { document.body.insertAdjacentHTML("beforeend", UTF8ToString($1)); } },  
 87431: () => { function rafLoop() { Module._animatefps(); requestAnimationFrame(rafLoop); } requestAnimationFrame(rafLoop); },  
 87544: ($0) => { window.addEventListener(UTF8ToString($0), function () { Module._handleEvent($0); }); },  
 87633: ($0, $1) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (el) el.textContent = UTF8ToString($1); },  
 87759: ($0, $1, $2) => { const el = document.getElementById(UTF8ToString($0)); if (!el) return; el.setAttribute(UTF8ToString($1), UTF8ToString($2)); },  
 87887: ($0, $1) => { const el = document.getElementById(UTF8ToString($0)); if (!el) return; el.removeAttribute(UTF8ToString($1)); },  
 88000: ($0, $1) => { el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (el) { el.attributes['data-ink-id'] = UTF8ToString($1); } },  
 88138: () => { Module._handleRoute(allocateUTF8(window.location.pathname)); window.addEventListener("popstate", () => { Module._handleRoute(allocateUTF8(window.location.pathname)); }); },  
 88312: ($0) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (el) el.remove(); },  
 88416: ($0, $1) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (el) el.outerHTML = UTF8ToString($1); },  
 88540: ($0, $1) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (el) el.outerHTML = UTF8ToString($1); },  
 88664: ($0, $1) => { const parent = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (parent) parent.insertAdjacentHTML('beforeend', UTF8ToString($1)); },  
 88821: ($0) => { const el = document.querySelector('[data-ink-id="' + UTF8ToString($0) + '"]'); if (el) el.remove(); },  
 88925: () => { return document.documentElement.clientHeight || document.body.clientHeight; },  
 89005: () => { return document.documentElement.clientWidth || document.body.clientWidth; },  
 89083: ($0, $1) => { window.wasmState = window.wasmState || {}; if (window.wasmState[UTF8ToString($0)] === undefined) { window.wasmState[UTF8ToString($0)] = $1; } },  
 89229: ($0, $1) => { window.wasmState = window.wasmState || {}; window.wasmState[UTF8ToString($0)] = $1; },  
 89317: ($0) => { window.wasmState = window.wasmState || {}; var val = window.wasmState[UTF8ToString($0)]; return (val === undefined) ? 0 : val; }
};
