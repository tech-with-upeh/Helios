...
var ASM_CONSTS = {
  85948: ($0) => { document.body.style = allocateUTF8(""); document.body.innerHTML = UTF8ToString($0); },  
 86036: ($0) => { document.title = UTF8ToString($0); },  
 86075: ($0) => { if (!document.getElementById("__ink_styles")) { const style = document.createElement("style"); style.id = "__ink_styles"; style.innerHTML = UTF8ToString($0); document.head.appendChild(style); } },  
 86273: ($0, $1) => { if ($1 == 1){ const script = document.createElement("script"); script.src = UTF8ToString($0); document.head.appendChild(script); } else { const linkElem = document.createElement("link"); linkElem.rel = "stylesheet"; linkElem.href = UTF8ToString($0); document.head.appendChild(linkElem); } },  
 86566: ($0) => { const linkElem = document.createElement("link"); linkElem.rel = "icon"; linkElem.href = UTF8ToString($0); linkElem.type = "image/x-icon"; document.head.appendChild(linkElem); },  
 86745: ($0, $1) => { document.body.setAttribute(UTF8ToString($0), UTF8ToString($1)); },  
 86813: ($0, $1) => { const id = UTF8ToString($0); if (!document.getElementById(id)) { document.body.insertAdjacentHTML("beforeend", UTF8ToString($1)); } },  
 86949: () => { function rafLoop() { Module._animatefps(); requestAnimationFrame(rafLoop); } requestAnimationFrame(rafLoop); },  
 87062: ($0) => { window.addEventListener(UTF8ToString($0), function () { Module._handleEvent($0); }); },  
 87151: () => { Module._handleRoute(allocateUTF8(window.location.pathname)); window.addEventListener("popstate", () => { Module._handleRoute(allocateUTF8(window.location.pathname)); }); },  
 87325: () => { return window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight; },  
 87427: () => { return window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth; },  
 87526: ($0, $1) => { window.wasmState = window.wasmState || {}; if (window.wasmState[UTF8ToString($0)] === undefined) { window.wasmState[UTF8ToString($0)] = $1; } },  
 87672: ($0, $1) => { window.wasmState = window.wasmState || {}; window.wasmState[UTF8ToString($0)] = $1; },  
 87760: ($0) => { window.wasmState = window.wasmState || {}; var val = window.wasmState[UTF8ToString($0)]; return (val === undefined) ? 0 : val; }
};
